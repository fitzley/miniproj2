...
#Note: Sunset color palette used across visuals.
sun_yellow <- "#FFC857"
sun_orange <- "#F77F00"
sunset_red <- "#D62828"
deep_blue <- "#003049"
...
#Note: Load project datasets using the exact filenames in the project folder.
health_data <- readr::read_csv("Chi_Health_Atlas_Data.csv")
dict_data <- readr::read_csv("CHA_Data_Dictionary(1).csv")
```
...
# Health Outcomes based on Particulate Matter: Hypertension
...
poverty_data <- read_csv("Chi_Health_Atlas_Data.csv")
```

```{r clean-data}
#Note: Clean and rename key variables for easier interpretation.
...
clean_data <- health_data %>%
  transmute(
    ...
    poverty = PMC_2020,
    median_income = `CHARIPZ_2023-2024`,
    lat = Latitude,
    lon = Longitude,
    pm25 = CHAKNKC_2023
  )

#Note: Keep compatibility with previous code.
...
map_df <- analysis_df %>%
  filter(!is.na(lat), !is.na(lon), !is.na(asthma))
...
scatter_df <- analysis_df %>%
  filter(!is.na(pm25), !is.na(asthma))

p <- ggplot(scatter_df, aes(pm25, asthma)) +
...
#Note: Predicted asthma is based on pm25 and population.

model <- lm(asthma ~ pm25 + population, data = analysis_df)

analysis_df <- analysis_df %>%
  mutate(
    predicted_asthma = predict(model, newdata = analysis_df),
    mismatch_index = asthma - predicted_asthma
  )
```
...
# Health Outcomes based on Pollution: Obesity
